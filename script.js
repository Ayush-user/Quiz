...
//Display reult and hide quiz container
...
//Initialize and start timer for current question
const startTimer=()=>{
    timer=setInterval(()=>{
        currentTime--;
        timerDisplay.textContent=`${currentTime}s`;
...
//     if (questions[i].category.toLowerCase() === quizCategory.toLowerCase()) {
//         categoryQuestions = questions[i]; // Store the first match and stop
...
const getRandomQuestion=()=>{
    ...
    //Show result if all questions are completed
    if(questionsIndexHistory.length>=Math.min(categoryQuestions.length,numberOfQuestions)) {
        return showQuizResult();
    }

    //Filter alredy asked questions and find rendom one
...
highlightCorrectAnswer=()=>{
    ...
    const inconHTML=`<span class="material-symbols-rounded">check_circle</span>`;
    correctOption.insertAdjacentHTML("beforeend", inconHTML );
...
const handleAnswer=(option,answerIndex)=>{
    ...
    //Inser icon based on correctness
    const inconHTML=`<span class="material-symbols-rounded">${isCorrect ? 'check_circle' :'cancel'} </span>`;
    option.insertAdjacentHTML("beforeend", inconHTML );
...
//Render current question and options in quiz
const renderQuestion=()=>{
    ...
    //Create option <li> elements and append them and handle event listner
